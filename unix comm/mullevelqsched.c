...
typedef struct mythread
{
  ...
  mythread thr[MAX],Main;
   mythread *readyq,*curr_thr,*fcfsq,*curr_thr2;
 ucontext_t ext;
...
void switcher()
{
 // printf("schedule called\n%d",count++);
  ...
  swapcontext(&temp->context,&curr_thr->context);
       ...
       {
          which=0;
         swapcontext(&curr_thr2->context,&curr_thr->context);
       }
   }
   else
  {
     setcontext(&curr_thr2->context);   which=1;  
  }
}
...
mythread * thr_create(void* fun,void* arg)
{
  ...
  {
      if(fcfsq==NULL) {fcfsq=t1; t1->next=t1; t1->prev=t1; curr_thr2=t1;}
       else {mythread *temp=fcfsq->prev; temp->next=t1; t1->prev=temp; fcfsq->prev=t1;t1->next=fcfsq;}
  }
   return t1;
}
...
void thr_exit()
{
 if(curr_thr==NULL)
   {
     if(curr_thr2->next==curr_thr2)
         {
            curr_thr2=NULL; 
         }
     else
        {
           mythread *temp=curr_thr2->next;
           curr_thr2=NULL;
           setcontext(&curr_thr2->context);  which=1;
        }
   }
